...
Al ampliar la región cercana a \(x\approx4.2,\)
...
\begin{itemize}
    \item Se requiere que \(f(x)\) sea continua en\( [x_l,x_u].\)
...
\begin{enumerate}
    \item Seleccionar \(x_l\) y \(x_u\) tales que \(f(x_l)f(x_u)<0.\)
...
Del análisis gráfico se obtuvo:\(c_l=12,\qquad c_u=16.\)
...
El primer punto medio es: \(c_r=\frac{c_l+c_u}{2}.\)

Sustituyendo:\(c_r=\frac{12+16}{2}=14.\)
...
La raíz se encuentra entre: \(14<c<15.\)
...
\begin{enumerate}
    ...
    \item \(f(x)\) es continua en \([x_l,x_u],\)
    \item y se cumple\(f(x_l)f(x_u)<0, \)
...
Sea el intervalo inicial:\(\Delta x_0
=
x_u^{(0)}
-
x_l^{(0)}.
...
\begin{frame}[fragile]{Código Python: Método de Bisección 1 /2}
...
\begin{frame}[fragile]{Código Python: Método de Bisección 2 /2}
...
def f(c):
    return (668.06 / c
        * (1 - math.exp(-0.146843 * c))
        - 40)
raiz, iteraciones, error = biseccion(f,12,16,es=0.5)
...
% ==========================
\begin{frame}{Cálculos con polinomios método de Horner}
  \textbf{Objetivo:} realizar operaciones básicas necesarias para los métodos de búsqueda de raíces, un polinomio de grado \(n\):      
    \(
...
Si se conoce una raíz aproximada \(t\) de un polinomio \(f(x)\), es útil \textbf{“sacarla”} del polinomio. Se divide \(f(x)\) entre el factor \((x - t)\) (o un factor cuadrático), reduciendo el grado. Esto evita volver a encontrar la misma raíz y simplifica el problema.
 
  \begin{block}{Idea básica}
    \begin{itemize}
      \item Representar el polinomio por sus coeficientes y aplicar una forma de \textbf{división sintética}:
        \[
          f(x) = (x - t)\,q(x) + r,
        \]
...
% ==================================================
\begin{frame}[fragile]{Pseudocódigo en Python: Método de Müller}
...
import cmath
def muller(f, xr, h=0.01, es=0.001, max_iter=100):
    x2 = xr
    x1 = xr + h*xr
    x0 = xr - h*xr
    for i in range(1, max_iter+1):
